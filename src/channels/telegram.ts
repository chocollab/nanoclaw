...
export function createTelegramBridge(options: TelegramBridgeOptions = {}): ChannelAdapter | null {
  ...
  return withTelegramActivity(
    wrapped,
    (threadId, messageId) => telegramAdapter.deleteMessage(threadId, messageId),
    readEnvFile(['TELEGRAM_ACTIVITY_ENABLED']).TELEGRAM_ACTIVITY_ENABLED !== 'false',
  );
}

/**
 * Turn id used by `ncl activity progress` (src/cli/resources/activity.ts).
 * One open "turn" per (platformId, threadId) is all this chat UX needs —
 * a chat processes one exchange at a time — so the producer never has to
 * compute or track a real per-turn id, it just always reports on this one.
 */
export const TELEGRAM_LIVE_TURN_ID = 'live';

/** Per-bot transient messages, isolated by chat and runner turn. */
export function withTelegramActivity(
  bridge: ChannelAdapter,
  deleteMessage: (threadId: string, messageId: string) => Promise<void>,
  enabled: boolean,
): ChannelAdapter {
  type Turn = { ids: string[]; timer?: NodeJS.Timeout; pending: Promise<void>; latest?: string };
  const turns = new Map<string, Turn>();
  const sendProgress = (entry: Turn, platformId: string, threadId: string | null, text: string): Promise<void> => {
    entry.pending = entry.pending.then(async () => {
      try {
        const id = await bridge.deliver(platformId, threadId, { kind: 'chat', content: { text: text.slice(0, 500) } });
        if (id) entry.ids.push(id);
      } catch {
        log.warn('Telegram progress delivery failed', { platformId });
      }
    });
    return entry.pending;
  };
  /** Delete every tracked breadcrumb for one turn and forget it. No-op if none is open. */
  const finishTurn = async (key: string, platformId: string, threadId: string | null): Promise<void> => {
    const entry = turns.get(key);
    if (!entry) return;
    clearTimeout(entry.timer);
    // A 60-second send may still be in flight when the final answer arrives.
    await entry.pending;
    for (const id of entry.ids) {
      try {
        await deleteMessage(threadId ?? platformId, id);
      } catch {
        // Never fail/retry the final reply due to permissions or an expired message.
        log.warn('Telegram progress deletion failed', { platformId, messageId: id });
      }
    }
    turns.delete(key);
  };
  return {
    ...
    async setTyping(platformId, threadId) {
      if (enabled) await bridge.setTyping?.(platformId, threadId);
    },
    async deliver(platformId, threadId, message) {
      const content = message.content as { text?: string; telegramActivity?: { phase: string; turnId: string } };
      const activity = content.telegramActivity;
      if (!activity) {
        const result = await bridge.deliver(platformId, threadId, message);
        // The real reply for this chat just went out — that's the natural
        // "done" signal, whether or not the agent ever called `ncl activity
        // progress`. Clears breadcrumbs even if the agent never sent a
        // `complete` itself. system/task_log rows are internal or
        // intermediate, not the turn's real answer, so they don't trigger it.
        if (enabled && message.kind !== 'system' && message.kind !== 'task_log') {
          await finishTurn(JSON.stringify([platformId, threadId, TELEGRAM_LIVE_TURN_ID]), platformId, threadId);
        }
        return result;
      }
      if (!enabled) return;
      const key = JSON.stringify([platformId, threadId, activity.turnId]);
      let entry = turns.get(key);
      if (activity.phase === 'complete') {
        await finishTurn(key, platformId, threadId);
        return;
      }
      if (!entry) {
        entry = { ids: [], pending: Promise.resolve() };
        turns.set(key, entry);
        const current = entry;
        current.timer = setTimeout(() => {
          void sendProgress(
            ...
            platformId,
            threadId,
            current.latest ? `Ще працюю: ${current.latest}` : 'Ще опрацьовую твій запит і готую відповідь.',
          );
        }, 60_000);
        current.timer.unref();
      }
      if (activity.phase === 'progress' && content.text) {
        entry.latest = content.text;
        await sendProgress(entry, platformId, threadId, content.text);
      }
    },
    async teardown() {
      for (const entry of turns.values()) clearTimeout(entry.timer);
      await Promise.all([...turns.values()].map((entry) => entry.pending));
      turns.clear();
